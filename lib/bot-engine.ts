...
export async function processIncomingWhatsAppMessage(
  ...
  const employee = db.getEmployeeByMobile(msg.fromMobile);
  ...
  let session = db.getSession(msg.fromMobile);
  if (!session) {
    session = {
  ...
  if (session.currentStep === 'COLLECT_REMARKS') {
    ...
    const todayVisits = db.getTodayVisitsByEmployee(employee.id);
    ...
    const allEmpVisits = db.getVisitsByEmployee(employee.id);
    ...
    db.clearSession(msg.fromMobile);
